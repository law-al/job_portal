...
import { X, Plus, Trash2, AlertCircle, ChevronRight } from 'lucide-react';
...
export default function AddJobPage() {
  ...
  const updateStageName = (id: string, name: string) => {
    setPipelineStages(pipelineStages.map((stage) => (stage.id === id ? { ...stage, name } : stage)));
  };

  const removeStage = (id: string) => {
    const updatedStages = pipelineStages.filter((stage) => stage.id !== id).map((stage, index) => ({ ...stage, order: index + 1 }));
  ...
  useEffect(() => {
    const fetchPipelineStages = async () => {
      if (!selectedPipeline || !session?.user?.companyId || !session?.user?.accessToken) {
        return;
      }
      ...
      try {
        const response = await fetchWithRetry({
          url: `jobs/${(session.user as any).companyId}/pipeline/stages?pipelineName=${encodeURIComponent(selectedPipeline)}`,
          options: {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (response.ok) {
          ...
          if (stages.length > 0) {
            setPipelineStages(
              stages.map((stage: { name: string; order: number }, index: number) => ({
                id: (index + 1).toString(),
                name: stage.name,
                order: stage.order,
              })),
  ...
  return (
    <>
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-4">
        <AppBreadCrumb items={[{ label: 'Jobs Management', href: '/admin/jobs' }, { label: 'Add New Job' }]} homeHref="/admin" className="mb-2" />
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-8">
            ...
            <form action={formAction} className="space-y-6">
              ...
              <div>
                ...
                />
                {getFieldError('title') && <p className="mt-1 text-sm text-red-600">{getFieldError('title')}</p>}
              </div>
              {/* Location & Job Type */}
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  />
                  {getFieldError('location') && <p className="mt-1 text-sm text-red-600">{getFieldError('location')}</p>}
                </div>
                <div>
                  <label htmlFor="jobType" className="block text-sm font-medium text-gray-900 mb-2">
                  ...
                  <select
                    ...
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white ${
                      getFieldError('jobType') ? 'border-red-300' : 'border-gray-300'
                    }`}
                  >
                  ...
                  {getFieldError('jobType') && <p className="mt-1 text-sm text-red-600">{getFieldError('jobType')}</p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <select
                    ...
                    required
                    className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white ${
                      getFieldError('experienceLevel') ? 'border-red-300' : 'border-gray-300'
                    }`}
                  >
                  ...
                  {getFieldError('experienceLevel') && <p className="mt-1 text-sm text-red-600">{getFieldError('experienceLevel')}</p>}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label htmlFor="applicationDeadline" className="block text-sm font-medium text-gray-900 mb-2">
                ...
                <div>
                  <label htmlFor="slot" className="block text-sm font-medium text-gray-900 mb-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mb-3">Select an existing pipeline or create a new one</p>
                ...
                {isLoadingStages && <div className="mb-4 text-sm text-gray-600">Loading pipeline stages...</div>}

                {/* Hidden input for pipelineName when existing pipeline is selected */}
                {selectedPipeline && selectedPipeline !== 'new' && <input type="hidden" name="pipelineName" value={selectedPipeline} />}

                <div className="space-y-3">
                  {pipelineStages.map((stage, index) => (
                    <div key={stage.id} className="flex items-center gap-3 p-3 border border-gray-300 rounded-lg bg-gray-50">
                      {/* Order Number */}
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-gray-600 w-6">{stage.order}.</span>
                      ...
                      <button
                        ...
                        disabled={pipelineStages.length <= 2 || isPipelineDisabled}
                      ...
                      <input type="hidden" name="stageOrder[]" value={stage.order} />
                    </div>
                  ))}
                </div>
                {/* Add Stage Button */}
              ...
              <div>
                ...
                />
                {getFieldError('description') && <p className="mt-1 text-sm text-red-600">{getFieldError('description')}</p>}
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  ...
                  />
                  <button type="button" onClick={addSkill} className="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition-colors">
                    Add
                  </button>
                </div>
                {skills.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {skills.map((skill, index) => (
                      <div key={index}>
                        <span key={index} className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-sm font-medium">
                          {skill}
